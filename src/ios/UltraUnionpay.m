...
@interface UltraUnionpay : CDVPlugin {}

@property(nonatomic, strong) NSString *ottPresetUrlSchemeForUnionPay;
...
@implementation UltraUnionpay

@synthesize ottPresetUrlSchemeForUnionPay;

- (void)pluginInitialize{
	
	NSString *unionPaySchemeId = @"ottultraunionpay";
	...
	[urlTypes indexOfObjectPassingTest:^BOOL(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
		if([unionPaySchemeId isEqualToString:[obj objectForKey:@"CFBundleURLName"]]){
			self.ottPresetUrlSchemeForUnionPay = [[obj objectForKey:@"CFBundleURLSchemes"] objectAtIndex:0];
			return YES;
		}
		return NO;
	}];
...
- (void)pay:(CDVInvokedUrlCommand*)command {
	...
	if(!fromScheme){
		fromScheme = self.ottPresetUrlSchemeForUnionPay;
	}
